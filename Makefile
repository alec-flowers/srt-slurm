...
setup:
	...
	if [ -f configs/nats-server ]; then \
		echo "✅ NATS already installed at configs/nats-server"; \
	else \
	...
	if [ -f configs/etcd ] && [ -f configs/etcdctl ]; then \
		echo "✅ ETCD already installed at configs/etcd"; \
	else \
	...
	if [ -f bin/uv ]; then \
		echo "✅ uv already installed at bin/uv ($$(file bin/uv | grep -o 'ARM aarch64\|x86-64'))"; \
	else \
		echo "⬇️  Downloading uv for $$ARCH_SHORT..."; \
		mkdir -p bin; \
