...
class TestManifestHashing:
    def test_large_files_are_counted_when_content_hash_is_skipped(self, tmp_path):
        large_file = tmp_path / "model.safetensors"
        with large_file.open("wb") as f:
            f.truncate(64 * 1024 * 1024 + 1)

        manifest = _relative_file_manifest(str(tmp_path))

        assert manifest["file_count"] == 1
        assert manifest["total_files_seen"] == 1
        assert manifest["total_bytes"] == large_file.stat().st_size
        assert manifest["hash"]["status"] == "partial"
        assert manifest["entries"][0]["hash"]["status"] == "skipped_large_file"

    def test_file_hash_errors_mark_manifest_partial_without_raising(self, tmp_path, monkeypatch):
        (tmp_path / "tokenizer.json").write_text("{}")

        def fail_hash(_path):
            raise OSError("read denied")

        monkeypatch.setattr("srtctl.core.lockfile._sha256_file", fail_hash)
        ...
        assert manifest["total_files_seen"] == 1
        assert manifest["hash"]["status"] == "partial"
        assert manifest["entries"][0]["hash"]["status"] == "error"
        assert "read denied" in manifest["entries"][0]["hash"]["error"]

    def test_manifest_entry_size_is_not_repeated_inside_complete_hash(self, tmp_path):
        file_path = tmp_path / "hf_quant_config.json"
        ...
        assert entry["hash"]["status"] == "complete"
        assert "size_bytes" not in entry["hash"]
...
class TestWriteLockfile:
    def test_preserves_recipe_text(self, tmp_path):
        ...
        lockfile_text = lockfile_path(tmp_path).read_text()
        assert lockfile_text.startswith('name: "test-job"')
        assert lockfile_path(tmp_path).parent == tmp_path / "reproduce"
    ...
    def test_has_lock_section(self, tmp_path):
        ...
        data = yaml.safe_load(lockfile_path(tmp_path).read_text())
        assert "lock" in data
    ...
    def test_has_comment_banner(self, tmp_path):
        ...
        text = lockfile_path(tmp_path).read_text()
    ...
    def test_rewrite_with_fingerprints(self, tmp_path):
        ...
        log_dir.mkdir()
        _write_fingerprint(fingerprints_dir(tmp_path) / "fingerprint_prefill_w0.json", hostname="p-node")
...
class TestLoadLockfileFingerprints:
    def test_from_new_lockfile_format(self, tmp_path):
        ...
        lockfile = tmp_path / "reproduce" / "recipe.lock.yaml"
    ...
    def test_from_dir_with_raw_fingerprints(self, tmp_path):
        _write_fingerprint(fingerprints_dir(tmp_path) / "fingerprint_w0.json", hostname="raw-node")

        fps = load_lockfile_fingerprints(tmp_path)
        assert fps["w0"]["hostname"] == "raw-node"

    def test_output_dir_does_not_read_legacy_logs_fingerprints(self, tmp_path):
        _write_fingerprint(tmp_path / "logs" / "fingerprint_w0.json", hostname="logs-node")

        assert load_lockfile_fingerprints(tmp_path) is None
...
class TestReproductionReport:
    ...
    @staticmethod
    def _artifact_manifest(file_count=2, total_files_seen=2, total_bytes=128, hash_value="abc"):
        return {
            "manifest": {
                "kind": "directory",
                ...
                "hash": {"status": "partial", "value": hash_value},
            }
        }
    ...
    def test_artifact_manifest_counts_are_compared(self):
        """Manifest count drift should be reported even when hashes are partial."""
        from srtctl.core.lockfile import generate_reproduction_report

        fp = self._minimal_fingerprint()
        prev_lock = {
            "slurm": {"job_id": "1234"},
            "fingerprints": {"agg_w0": fp},
            "artifacts": {
                "model": self._artifact_manifest(file_count=2, total_files_seen=2, total_bytes=128),
                "mounts": [
                    {
                        "target": "/traces",
                        **self._artifact_manifest(file_count=4, total_files_seen=4, total_bytes=4096),
                    }
                ],
            },
        }
        new_artifacts = {
            "model": self._artifact_manifest(file_count=3, total_files_seen=3, total_bytes=256, hash_value="def"),
            "mounts": [
                {
                    "target": "/traces",
                    **self._artifact_manifest(file_count=5, total_files_seen=5, total_bytes=4096),
                }
            ],
        }

        summary, report, issues = generate_reproduction_report(
            prev_lock,
            {"agg_w0": fp},
            new_artifacts=new_artifacts,
        )

        assert any("artifact model file_count" in issue for issue in issues)
        assert any("artifact mount:/traces total_files_seen" in issue for issue in issues)
        assert any("Artifacts:" in line for line in report)
        assert any("artifact model file_count" in line for line in summary)
        assert any("ISSUES FOUND" in line for line in report)

    def test_matching_artifact_manifests_stay_clean(self):
        from srtctl.core.lockfile import generate_reproduction_report

        fp = self._minimal_fingerprint()
        artifacts = {
            "model": self._artifact_manifest(),
            "mounts": [{"target": "/traces", **self._artifact_manifest(hash_value="mount-hash")}],
        }
        prev_lock = {
            "slurm": {"job_id": "1234"},
            "fingerprints": {"agg_w0": fp},
        ...
        _summary, report, issues = generate_reproduction_report(
            prev_lock,
            {"agg_w0": fp},
